...
export default function VaporizeTextCycle({
  ...
  const [animationState, setAnimationState] = useState<"static" | "vaporizing" | "fadingIn" | "waiting" | "finished">("static");
  ...
  useEffect(() => {
    ...
    } else if (!isInView && !runOnce) {
      // When component goes out of view, reset to static state (only if not runOnce)
...
// SEO ELEMENT
// ------------------------------------------------------------ //
const SeoElement = memo(({ tag = Tag.SPAN, texts }: { tag: Tag, texts: string[] }) => {
  const style = useMemo(() => ({
  ...
  const safeTag = Object.values(Tag).includes(tag) ? tag : "span";
...
const renderCanvas = ({
  ...
  const currentText = framerProps.texts[currentTextIndex] || "21 days";
